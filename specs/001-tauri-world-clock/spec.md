...
- **FR-027**: System MUST support basic keyboard navigation for all primary actions (add/remove zones, create/manage alarms, access settings) on desktop platforms. *(See FR-046 for concrete shortcut bindings that implement a subset of this requirement.)*
...
- **FR-046**: Desktop platforms MUST support keyboard shortcuts: Cmd/Ctrl+N (add timezone), Cmd/Ctrl+K (focus search), Cmd/Ctrl+, (open settings), Cmd/Ctrl+Q (quit), Escape (close modal/dialog). Shortcuts MUST be displayed in menu items and discoverable via tooltips. *(Implements the shortcut subset of FR-027.)*
